...
def process_inputs() -> str:
    """Handles different input modes and returns the unified raw text string."""

    st.markdown("""
        <p style='color:#6b7280; font-size:0.8rem; letter-spacing:0.08em; text-transform:uppercase; font-weight:600; margin-bottom:8px;'>
            Select Input Mode
        </p>
    """, unsafe_allow_html=True)
    ...
    if input_mode == "✏️ Text":
        ...
        if user_input:
            raw_text = TextProcessor.process_text(user_input)
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("🔍 Extracting text via OCR..."):
                ...
                if extracted:
                    ...
                    raw_text = st.text_area("Review & Edit OCR Output:", value=extracted, height=100)
                else:
        ...
        st.markdown("""
            <p style='color:#9ca3af; font-size:0.85rem; margin-bottom:8px;'>
                🎤 Click the button below to record your math problem aloud.
            </p>
        """, unsafe_allow_html=True)

        # Live microphone recording via st.audio_input
        ...
        if audio_source is not None:
            ...
            with st.spinner("🎧 Transcribing audio via Whisper..."):
                ...
                if extracted:
                    ...
                    raw_text = st.text_area("Review & Edit Transcript:", value=extracted, height=100)
                else:
                    st.error("❌ Failed to process audio.")

    return input_mode, raw_text
